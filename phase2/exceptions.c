...
void passUpordie(int exception, state_t*);
...
static void syscallHandler(state_t* state) {
  ...
  // TODO: devo controllare anche se il codice e' troppo basso? (non esiste il servizio)
  ...
  switch (syscall_code) {
    case CREATEPROCESS:  // this call creates a new process
      ...
      klog_print("-PC:");
      klog_print_hex(newPCB->p_s.pc_epc);
    ...
    case PASSEREN:
      ...
      if (*semAdd1 < 0) {                  // if the semaphore address is negative, its value can't be decreased
        ...
        klog_print("-pcBlocked:");
        klog_print_hex(current->p_s.pc_epc);
...
// Change: gli utbRefill non vengono gestiti dall'exception handler
void passUpordie(int exception, state_t* exc_state) {
  klog_print("-pud");
  ACQUIRE_LOCK(&global_lock);
  int cpu_id = getPRID();
